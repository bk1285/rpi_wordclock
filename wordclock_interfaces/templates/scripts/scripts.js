...
new Vue(
{
	...
	methods: {
		loadApi: function () {
			this.$http.get('/api/plugin').
			  then(this.successCallback2, this.errorCallback);
		...
		successCallback2: function(response) {
		...
		setColourWheel: function(wheel) {
			var r,g,b,type;
			[g,r,b] = hslToRgb(wheel.hue/360, wheel.saturation/100, wheel.luminosity/100);
	...
	created() {
		setTimeout(() => {
			this.selectedPlugin = this.currentPlugin.name;
